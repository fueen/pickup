...
# v1.10 — Review 缩放、Tab 胶囊、最近删除还原与滑动提示精简

> 版本：v1.10 | 日期：2026-05-31 | 状态：待开发

## 概述

基于当前 v1.9 之后的新一轮体验反馈，聚焦 4 个方向：Review 页面照片缩放能力补强、底部三功能 Tab 栏视觉升级、最近删除页支持多选还原、以及浏览页左滑时去掉"上一张"文字提示。目标是在不改变现有核心流程的前提下，提高确认删除前的安全感、底部导航的精致度，以及最近删除记录的可操作性。

---

## 需求清单

### REQ-01 Review 页面照片支持双指捏合放大/缩小

**现状**：Review 页面已接入 `PhotoZoomModal`，但需要明确验收目标：用户在照片 Review 场景中必须可以通过双指捏合对照片进行放大和缩小，用于确认细节、避免误删。

**目标**：在 Review 页面查看照片时，支持自然的双指捏合缩放体验；放大后可以查看细节，缩小后回到完整照片视图。

**实现要点**：

- **入口**：在 `DeleteGrid` 中点击或长按照片进入全屏预览；保留当前勾选/取消删除的交互，不产生误触
- **缩放能力**：全屏预览内支持双指捏合缩放，范围建议为 `1x ~ 5x`
- **缩小回弹**：缩放比例小于 `1x` 时回弹到原始大小，避免图片变成过小悬浮状态
- **放大后浏览**：照片放大后支持拖动/平移查看局部区域
- **关闭方式**：单击空白处或顶部关闭按钮关闭预览；关闭后回到原 Review 网格，保持已勾选状态不变
- **平台兼容**：Android / iOS 表现一致；优先复用现有 `PhotoZoomModal`
...
### REQ-02 底部三功能 Tab 栏增加胶囊形容器包裹

**现状**：底部 Tab 栏由 `_layout.tsx` 中的 `SimpleTabBar` 独立渲染，当前可以完成三项功能入口导航，但视觉上仍偏平，缺少整体容器感。

**目标**：在底部三个功能区 Tab 图标外层增加一个胶囊形容器，将三枚入口统一包裹起来，使底部导航更像一个精致的浮动控制条。

**实现要点**：

- **容器形态**：三枚 Tab 外层增加横向胶囊容器，`borderRadius` 采用大圆角，整体浮于底部安全区上方
- **视觉风格**：延续当前黑金风格，建议使用半透明深色背景、细描边、轻微阴影/发光，不使用过重装饰
- **布局**：三个功能入口在胶囊内等距排列，图标视觉居中，触控区域不小于 `44x44`
- **选中态**：当前 Tab 使用金色强调，可配合轻微缩放或高亮底圆；未选中为低对比灰色
- **安全区**：兼容 Android 手势导航和 iOS 安全区，不遮挡页面底部按钮
- **现有问题规避**：继续使用独立 `SimpleTabBar`，不要恢复默认 tab bar，避免历史上的幽灵点击区域和图标偏移问题
...
### REQ-03 个人中心最近删除支持多选与还原

**现状**：个人中心点击"最近删除"进入 `app/recent-deletes.tsx` 后，当前页面以网格展示本 App 删除过的照片缓存缩略图，主要用于回溯查看；如果原照片已从系统最近删除中被清理，App 不应继续展示无法操作的记录。

**目标**：最近删除页进入选择模式后，点击照片右上角出现打勾形状，可多选；下方出现"还原照片"操作区；点击还原后将选中的照片恢复到系统相册。若用户已经在系统最近删除中彻底清理了对应照片，则当前界面不展示该照片记录。

**实现要点**：

- **选择模式**：
  - 点击照片进入选中态，缩略图右上角显示圆形打勾标记
  - 支持再次点击取消选中
  - 支持多选，页面标题或底部操作区显示已选数量
- **底部操作区**：
  - 当 `selectedIds.size > 0` 时，从底部出现操作栏
  - 主操作为"还原照片"
  - 操作栏需避开系统安全区，样式与现有暗色/金色体系一致
- **还原逻辑**：
  - 优先使用缓存缩略图/缓存文件创建新的媒体资产，还原到系统相册
  - 还原成功后，从最近删除记录中移除对应项，并刷新统计/列表
  - 若部分照片还原失败，保留失败项并给出 Toast 提示
- **系统最近删除校验**：
  - 页面加载时校验每条记录对应的可还原资源是否仍存在
  - 如果用户已在系统中直接清理了最近删除照片，当前界面过滤掉对应记录
  - 过滤后的无效记录应从本地 `recentDeletes` 持久化中清理，避免下次再次出现
- **边界说明**：
  - Android 的系统删除行为可能是永久删除，不一定存在系统最近删除相册；还原能力以 App 删除前保存的缓存文件为准
  - 如果缓存文件也不存在，则该记录视为不可还原，页面不展示
...
### REQ-04 浏览页左滑时去掉"上一张"文字展示

**现状**：浏览页滑动卡片时，`ActionIndicator` 会在水平滑动过程中展示导航提示文字，其中包含"上一张 →"。用户希望左滑时不再出现"上一张"文字展示，减少干扰。

**目标**：左滑操作过程中去掉"上一张"文字提示，仅保留必要的手势反馈或卡片动画，不再展示这段文字。

**实现要点**：

- **移除文案**：从 `ActionIndicator` 中去掉"上一张 →"文本渲染
- **交互不变**：只移除提示文案，不改变左滑/右滑的实际导航行为
- **视觉反馈**：保留卡片位移、旋转、缩放等动效；如需提示，可仅保留低干扰图标或完全不显示
- **新手引导**：`GestureGuideOverlay` 中的手势说明可保留"右滑 · 上一张"，因为这是教学层，不属于滑动时的即时提示
...
| 优先级 | 需求编号 | 说明 |
| ------ | -------- | ---- |
| P0 | REQ-03 | 最近删除还原是核心功能升级，涉及数据有效性与用户信任 |
| P1 | REQ-01 | Review 缩放直接影响误删风险，需要确保体验可靠 |
| P1 | REQ-02 | 底部导航视觉升级，提升整体完成度 |
| P2 | REQ-04 | 文案去噪，属于交互细节打磨 |
...
1. Review 页面打开照片预览后，双指捏合可以平滑放大/缩小照片，缩放范围约为 `1x ~ 5x`
2. 放大后的照片可拖动查看局部，关闭预览后 Review 网格的勾选状态不丢失
3. 底部三个 Tab 入口被统一包裹在胶囊形浮动容器内，选中态清晰，触控区域正常
4. 最近删除页支持多选，选中照片右上角显示打勾标记
5. 选中至少 1 张照片后，底部出现"还原照片"操作区
6. 点击"还原照片"后，成功还原的照片从最近删除页移除，并可在系统相册中看到恢复后的照片
7. 如果用户已在系统中彻底清理某张最近删除照片，进入最近删除页时不展示该照片
8. 浏览页滑动过程中不再展示"上一张 →"文字，滑动行为本身保持不变
