  ...
  switch (action.type) {
    ...
    case 'MARK_DELETE':
      return {
        ...
        interactionLog: [...state.interactionLog, { photoId: action.payload, action: 'delete', timestamp: Date.now() }],
      };
    case 'MARK_KEEP':
      return {
        ...state,
        markedKeepIds: [...state.markedKeepIds, action.payload],
        interactionLog: [...state.interactionLog, { photoId: action.payload, action: 'keep', timestamp: Date.now() }],
      };
    case 'SKIP':
      return {
        ...state,
        skippedIds: [...state.skippedIds, action.payload],
        interactionLog: [...state.interactionLog, { photoId: action.payload, action: 'skip', timestamp: Date.now() }],
      };
    case 'UNDO_LAST': {
